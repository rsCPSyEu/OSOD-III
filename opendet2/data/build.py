...
def get_detection_dataset_dicts(names, filter_empty=True, min_keypoints=0, proposal_files=None, cfg=None):
    ...
    # # modify category_id according to hierarchical classes
    # if cfg.HIE_CLF.ENABLE:
    #     dataset_dicts = add_hierarchical_classes(dataset_dicts)
    ...
    d_name = names[0]
    if d_name == 'coco_2017_val':
        pass
    elif 'train' in d_name or 'val' in d_name:
...
def remove_unk_instances(cfg, dataset_dicts):
    ...
    for entry in copy.copy(dataset_dicts):
        ...
        for annotation in copy.copy(annos):
            if annotation["category_id"] not in valid_classes: # category_id is already converted into "index"
